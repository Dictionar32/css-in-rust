...
// ── New Rust functions (ported from JS compiler/engine) ───────────────────────

/**
 * Extract semua JSX component usage dari source code.
 * Menggantikan extractComponentUsage di deadStyleEliminator.ts.
 * @param {string} source  Source code TSX/JSX
 * @returns {Array<{component: string, propsJson: string}>}
 */
export function extractComponentUsageNative(source) {
  return callOptional("extractComponentUsage", source);
}

/**
 * Normalize, deduplicate, dan sort class list.
 * Menggantikan normalizeClasses + Set dedup di classMerger.ts.
 * @param {string} raw  Raw class string (boleh ada duplikat/whitespace)
 * @returns {{normalized: string, duplicatesRemoved: number, uniqueCount: number}}
 */
export function normalizeAndDedupClassesNative(raw) {
  return callOptional("normalizeAndDedupClasses", raw);
}

/**
 * Hitung diff antara dua class lists.
 * Dipakai oleh incremental compiler untuk deteksi perubahan.
 * @param {string[]} previous
 * @param {string[]} current
 * @returns {{added: string[], removed: string[], unchanged: string[], hasChanges: boolean}}
 */
export function diffClassListsNative(previous, current) {
  return callOptional("diffClassLists", previous, current);
}

/**
 * Extract classes dari banyak files sekaligus secara parallel (Rayon).
 * Menggantikan loop sequential di scanner.
 * @param {string[]} filePaths  Absolute paths
 * @returns {Array<{file: string, classes: string[], contentHash: string, ok: boolean, error?: string}>}
 */
export function batchExtractClassesNative(filePaths) {
  return callOptional("batchExtractClasses", filePaths);
}

/**
 * Cek daftar classes terhadap safelist (O(n) via HashSet).
 * Menggantikan Array.filter().includes() O(n*m) di JS.
 * @param {string[]} classes
 * @param {string[]} safelist
 * @returns {{matched: string[], unmatched: string[], safelistSize: number}}
 */
export function checkAgainstSafelistNative(classes, safelist) {
  return callOptional("checkAgainstSafelist", classes, safelist);
}

// ── Batch 2: staticVariantCompiler, componentHoister, styleBucket, analyzer ──

/**
 * Deteksi dan hoist tw component declarations ke module scope.
 * Menggantikan hoistComponents() di componentHoister.ts.
 * @param {string} source
 * @returns {{ code: string, hoisted: string[], warnings: string[] }}
 */
export function hoistComponentsNative(source) {
  return callOptional("hoistComponents", source);
}

/**
 * Compile semua kombinasi variant ke lookup table (O(1) runtime).
 * Menggantikan compileAllVariantCombinations() di staticVariantCompiler.ts.
 * @param {string} configJson  JSON StaticVariantConfig
 * @returns {{ id, tableJson, keys, defaultKey, combinations }}
 */
export function compileVariantTableNative(configJson) {
  return callOptional("compileVariantTable", configJson);
}

/**
 * Classify dan sort classes berdasarkan CSS property bucket.
 * Menggantikan classifyIntoBuckets() + sortByBucket() di styleBucketSystem.ts.
 * @param {string[]} classes
 * @returns {Array<{ className, bucket, sortOrder }>}
 */
export function classifyAndSortClassesNative(classes) {
  return callOptional("classifyAndSortClasses", classes);
}

/**
 * Parse dan merge CSS declarations dari banyak CSS chunks.
 * Menggantikan mergeDeclarationMap() + declarationMapToString() di classToCss.ts.
 * @param {string[]} cssChunks
 * @returns {{ declarationsJson, declarationString, count }}
 */
export function mergeCssDeclarationsNative(cssChunks) {
  return callOptional("mergeCssDeclarations", cssChunks);
}

/**
 * Analisis class usage dari scan result — batch mode.
 * Menggantikan BundleAnalyzer.analyzeClass() di bundleAnalyzer.ts.
 * @param {string[]} classes
 * @param {string} scanResultJson  JSON ScanWorkspaceResult
 * @param {string} css
 * @returns {Array<{ className, usageCount, filesJson, bundleSizeBytes, isDeadCode }>}
 */
export function analyzeClassUsageNative(classes, scanResultJson, css) {
  return callOptional("analyzeClassUsage", classes, scanResultJson, css);
}

// ── Batch 3: semantic, CSS reverse lookup, parseClasses ───────────────────────

/**
 * Parse raw class string — filter invalid tokens, deduplicate.
 * Menggantikan parseClasses() di syntax/src/index.ts.
 * @param {string} raw
 * @returns {string[]}
 */
export function parseClassesFromStringNative(raw) {
  return callOptional("parseClassesFromString", raw);
}

/**
 * Deteksi konflik antara Tailwind classes (misal bg-red + bg-blue).
 * Menggantikan detectConflicts() di analyzer/src/semantic.ts.
 * @param {string} usagesJson  JSON: Array<{name: string, count: number}>
 * @returns {{ conflicts: Array<{group,variantKey,classes,message}>, conflictedClassNames: string[] }}
 */
export function detectClassConflictsNative(usagesJson) {
  return callOptional("detectClassConflicts", usagesJson);
}

/**
 * Classify classes sebagai known/unknown Tailwind utilities — batch mode.
 * Menggantikan isKnownTailwindClass() + utilityPrefix() di semantic.ts.
 * @param {string[]} classes
 * @param {string[]} safelist
 * @param {string[]} customUtilities
 * @returns {Array<{className,isKnown,variantKey,baseClass,utilityPrefix,isArbitrary}>}
 */
export function classifyKnownClassesNative(classes, safelist, customUtilities) {
  return callOptional("classifyKnownClasses", classes, safelist, customUtilities);
}

/**
 * Parse CSS string menjadi rule list untuk reverse lookup.
 * Menggantikan parseCSS() + findByProperty() di reverseLookup.ts.
 * @param {string} css
 * @returns {Array<{className,property,value,isImportant,variants,specificity}>}
 */
export function parseCssRulesNative(css) {
  return callOptional("parseCssRules", css);
}

/**
 * Split classes menjadi komponen: variant, base, modifier — batch mode.
 * Menggantikan splitVariantAndBase() + utilityPrefix() di semantic.ts.
 * @param {string[]} classes
 * @returns {Array<{variantKey,base,variants,isArbitrary,hasModifier,modifier}>}
 */
export function batchSplitClassesNative(classes) {
  return callOptional("batchSplitClasses", classes);
}

// ── Batch 4: cssToIr, bundleAnalyzer, impactTracker, routeAnalysis ───────────

/**
 * Parse CSS string menjadi flat rule list — full selector + declaration parsing.
 * Menggantikan parseRules() + parseSelector() di cssToIr.ts.
 * @param {string} css
 * @param {string|null} prefix
 * @returns {Array<{className,property,value,important,variants,pseudoClasses,mediaQuery,specificity,layer}>}
 */
export function parseCssToRulesNative(css, prefix = null) {
  return callOptional("parseCssToRules", css, prefix);
}

/**
 * Hitung bundle contribution (size, variants, deps) untuk banyak classes sekaligus.
 * Menggantikan calculateBundleContribution() + extractVariantChains() + extractDependencies().
 * @param {string[]} classes
 * @param {string} css
 * @returns {Array<{className,sizeBytes,variantChains,dependencies,inCss}>}
 */
export function calculateBundleContributionsNative(classes, css) {
  return callOptional("calculateBundleContributions", classes, css);
}

/**
 * Deteksi dead code antara CSS output dan scan result — O(n+m) via HashSet.
 * Menggantikan detectDeadCode() + checkIsDeadCode() di bundleAnalyzer.ts.
 * @param {string} scanResultJson  JSON ScanWorkspaceResult
 * @param {string} css
 * @returns {{deadInCss,deadInSource,liveClasses,totalCssClasses,totalSourceClasses}}
 */
export function detectDeadCodeNative(scanResultJson, css) {
  return callOptional("detectDeadCode", scanResultJson, css);
}

/**
 * Hitung impact score per class (usage × size weighted).
 * Menggantikan logic di impactTracker.ts + bundleAnalyzer.ts.
 * @param {string[]} classes
 * @param {string} scanResultJson
 * @param {string} css
 * @param {number} [usageWeight=0.6]
 * @param {number} [sizeWeight=0.4]
 * @returns {Array<{className,usageScore,sizeScore,impactScore,usageCount,sizeBytes}>}
 */
export function calculateImpactScoresNative(classes, scanResultJson, css, usageWeight, sizeWeight) {
  return callOptional("calculateImpactScores", classes, scanResultJson, css, usageWeight ?? 0.6, sizeWeight ?? 0.4);
}

/**
 * Analisis distribusi classes per route untuk CSS code splitting.
 * Menggantikan routeCssCollector.ts logic.
 * @param {string} routeFilesJson  JSON Record<route, string[]>
 * @param {string} scanResultJson
 * @returns {Array<{route,classes,exclusiveClasses,classCount}>}
 */
export function analyzeRouteClassDistributionNative(routeFilesJson, scanResultJson) {
  return callOptional("analyzeRouteClassDistribution", routeFilesJson, scanResultJson);
}
