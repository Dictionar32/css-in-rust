...
/** Apply class diff: (existing ∪ added) ∖ removed. Result is sorted. *\/
export declare function applyClassDiff(
  existing: Array<string>,
  added: Array<string>,
  removed: Array<string>
): Array<string>
 
/** Check if two class arrays contain identical elements (order-independent). *\/
export declare function areClassSetsEqual(a: Array<string>, b: Array<string>): boolean
 
/**
 * Rebuild workspace result from a list of scanned files.
 * Deduplicates and sorts uniqueClasses in one Rust pass.
 *\/
...
/** Hash a file by path — dipakai untuk cache invalidation. */
export declare function hashContent(
  content: string,
  algorithm?: "md5" | "sha256" | "fnv" | null,
  length?: number | null,
): string
 
/**
 * Read a file and hash its content in one native call.
 * Replaces `hashFile()` in `shared/src/hash.ts`.
 * Returns "00000000" if the file cannot be read.
 *
 * @param algorithm "md5" (default) | "sha256" | "fnv"
 * @param length    Truncate output to this many hex chars (default: full length)
 */
...
/**
 * Scan a file: read + extract Tailwind classes + hash in one native call.
 * More efficient than calling scanFile + hashContent separately.
 * Returns null if the file cannot be read.
 */
export declare function scanFileNative(filePath: string): NativeScanFileResult | null
 
/**
 * Batch scan multiple files in parallel (rayon par_iter).
 * Files that fail to read return with empty classes and empty hash.
 */
