...
function resolveCompound(
  compounds: Array<{ class: string } & CompoundCondition>,
...
export function compileAllVariantCombinations(config: StaticVariantConfig): CompiledVariantTable {
  ...
  if (native?.compileVariantTableNative) {
    const r = native.compileVariantTableNative(JSON.stringify(config)) as {
      id: string; tableJson: string; keys: string[]; defaultKey: string; combinations: number
    } | null
    if (r) {
      return {
        id: r.id,
        table: JSON.parse(r.tableJson ?? "{}") as Record<string, string>,
        keys: r.keys,
...
export class StaticVariantResolver {
  ...
  resolve(props: Record<string, string>): string {
