...
// ─── Transforms specific primitives ────────────────────────────────────────────
export const TransformBox = tw.div({
    base: "w-20 h-20 rounded-xl bg-indigo-500 flex items-center justify-center text-white text-xs font-bold cursor-pointer select-none transition-transform duration-300",
    variants: {
        preset: {
            none: "",
            translate: "hover:translate-x-4",
            rotate: "hover:rotate-45",
            scale: "hover:scale-125",
            skew: "hover:[transform:skewX(15deg)]",
        },
    },
    defaultVariants: { preset: "none" },
})

export const SceneWrap = tw.div({
    base: "rounded-xl border border-[color-mix(in_srgb,var(--foreground)_10%,transparent)] bg-[var(--surface)] p-6 my-4 flex items-center justify-center min-h-[160px] overflow-hidden",
})

export const Perspective3D = tw.div({
    base: "relative w-24 h-24 cursor-pointer select-none",
    variants: {
        style: {
            card: "[transform-style:preserve-3d] transition-transform duration-500 hover:[transform:rotateY(180deg)]",
        },
    },
