...
// ─── Visual Effects specific primitives ────────────────────────────────────────
export const FilterBox = tw.div({
    base: "w-24 h-24 rounded-xl overflow-hidden shrink-0 transition-all duration-300",
    variants: {
        filter: {
            none: "",
            ...
            dropshadow: "[filter:drop-shadow(4px_4px_8px_rgba(99,102,241,0.6))]",
            grayscale: "[filter:grayscale(1)]",
        },
    },
    defaultVariants: { filter: "none" },
})

export const BlendBox = tw.div({
    base: "w-24 h-24 rounded-xl flex items-center justify-center font-bold text-sm",
    variants: {
        blend: {
    ...
    defaultVariants: { blend: "normal" },
})

export const ClipBox = tw.div({
    base: "w-32 h-32 bg-gradient-to-br from-indigo-500 to-pink-500 transition-all duration-300",
})
