...
// ─── Transitions & Animations specific primitives ──────────────────────────────
export const AnimBox = tw.div({
    base: "w-16 h-16 rounded-xl flex items-center justify-center text-white font-bold text-sm cursor-pointer select-none",
    variants: {
        color: {
...
export const TimingRow = tw.div({ base: "flex items-center gap-4 py-2" })
export const TimingLabel = tw.span({ base: "text-xs font-mono w-32 shrink-0 text-[color-mix(in_srgb,var(--foreground)_55%,transparent)]" })
export const TimingBar = tw.div({ base: "h-2 rounded-full bg-indigo-500 transition-[width]" })
