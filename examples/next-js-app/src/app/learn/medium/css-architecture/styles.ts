...
export const Callout = tw.div({ base: "rounded-xl border px-4 py-3 my-5 text-sm leading-relaxed flex gap-3", variants: { type: { note: "bg-blue-50 border-blue-200 text-blue-900", tip: "bg-emerald-50 border-emerald-200 text-emerald-900", warning: "bg-amber-50 border-amber-200 text-amber-900", php: "bg-violet-50 border-violet-200 text-violet-900", danger: "bg-red-50 border-red-200 text-red-900" } }, defaultVariants: { type: "note" }, sub: { "span:icon": "text-base shrink-0 mt-0.5", "div:content": "flex-1", "strong:title": "block font-semibold mb-0.5" } })
...
// ─── CSS Architecture specific primitives ──────────────────────────────────────
export const LayerBadge = tw.span({
    base: "inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono font-medium",
    variants: {
        layer: {
            base: "bg-slate-100 text-slate-700 border border-slate-200",
            components: "bg-indigo-100 text-indigo-700 border border-indigo-200",
            utilities: "bg-emerald-100 text-emerald-700 border border-emerald-200",
            theme: "bg-amber-100 text-amber-700 border border-amber-200",
        },
    },
    defaultVariants: { layer: "base" },
})

export const ColDemo = tw.div({
    base: "rounded-xl border border-[color-mix(in_srgb,var(--foreground)_10%,transparent)] bg-[var(--surface)] p-5 my-4",
    variants: {
        cols: {
            "2": "[column-count:2] [column-gap:1.5rem]",
            "3": "[column-count:3] [column-gap:1.5rem]",
            "4": "[column-count:4] [column-gap:1rem]",
        },
    },
    defaultVariants: { cols: "3" },
})

export const IntrinsicBox = tw.div({
    base: "rounded-lg border border-[color-mix(in_srgb,var(--foreground)_10%,transparent)] bg-indigo-50 text-indigo-800 px-3 py-2 text-xs font-mono",
    variants: {
        sizing: {
            "min-content": "w-min",
            "max-content": "w-max",
    ...
    defaultVariants: { sizing: "fit-content" },
})
