...
// ─── Colors & Gradients specific primitives ────────────────────────────────────
export const ColorSwatch = tw.div({
    base: "w-10 h-10 rounded-lg border border-[color-mix(in_srgb,var(--foreground)_10%,transparent)] shrink-0",
})

export const GradientBox = tw.div({
    base: "h-16 rounded-xl w-full transition-all duration-300",
    variants: {
        gradient: {
            linear: "bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500",
            radial: "[background:radial-gradient(circle,#6366f1,#ec4899)]",
            conic: "[background:conic-gradient(from_0deg,#6366f1,#ec4899,#6366f1)]",
            repeating: "[background:repeating-linear-gradient(45deg,#6366f1_0px,#6366f1_10px,#c7d2fe_10px,#c7d2fe_20px)]",
        },
    },
    defaultVariants: { gradient: "linear" },
})

export const ColorRow = tw.div({ base: "flex items-center gap-3 py-1.5" })
export const ColorLabel = tw.span({ base: "text-xs font-mono text-[color-mix(in_srgb,var(--foreground)_55%,transparent)] min-w-[160px]" })
