...
export const MethodCard = tw.div({
    ...
    variants: {
        method: {
            bem: "border-blue-200",
            cube: "border-violet-200",
            utility: "border-emerald-200",
            token: "border-amber-200",
        },
    },
    defaultVariants: { method: "bem" },
    sub: {
        "span:tag": "inline-block text-[10px] font-bold px-2 py-0.5 rounded-full mb-2",
        "h3:title": "text-sm font-bold mb-1",
        "p:desc": "text-xs text-[color-mix(in_srgb,var(--foreground)_60%,transparent)] leading-relaxed",
    },
})

export const MethodGrid = tw.div({ base: "grid grid-cols-1 sm:grid-cols-2 gap-3 my-5" })

export const TokenRow = tw.div({ base: "flex flex-col gap-2 my-4" })
export const TokenItem = tw.div({
    base: "flex items-center gap-3 p-2.5 rounded-lg border text-xs font-mono",
    variants: {
        layer: {
            primitive: "bg-blue-50 border-blue-200 text-blue-800",
            semantic: "bg-violet-50 border-violet-200 text-violet-800",
...
export const CompareRow = tw.tr({ base: "border-b border-[color-mix(in_srgb,var(--foreground)_8%,transparent)]" })
export const CompareTh = tw.th({ base: "text-left py-2 px-3 font-semibold text-[color-mix(in_srgb,var(--foreground)_60%,transparent)] bg-[color-mix(in_srgb,var(--foreground)_3%,transparent)]" })
export const CompareTd = tw.td({ base: "py-2 px-3 text-[color-mix(in_srgb,var(--foreground)_75%,transparent)]" })
