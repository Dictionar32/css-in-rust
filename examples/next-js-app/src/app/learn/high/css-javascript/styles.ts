...
// ─── CSS-JavaScript specific primitives ───────────────────────────────────────
export const AnimBox = tw.div({
    base: "w-16 h-16 rounded-xl flex items-center justify-center text-xl font-bold text-white transition-all",
    variants: {
        color: {
...
export const EventLog = tw.div({
    base: "rounded-xl border border-[color-mix(in_srgb,var(--foreground)_10%,transparent)] bg-[var(--surface)] overflow-hidden",
    sub: {
        header: "px-3 py-2 text-[10px] font-semibold uppercase tracking-widest border-b border-[color-mix(in_srgb,var(--foreground)_8%,transparent)] bg-[color-mix(in_srgb,var(--foreground)_3%,transparent)]",
        "ul:list": "p-3 space-y-1 max-h-40 overflow-y-auto",
        "li:item": "text-[11px] font-mono text-[color-mix(in_srgb,var(--foreground)_65%,transparent)]",
    },
})

export const Slider = tw.input({ base: "w-full accent-[var(--accent)] cursor-pointer" })

export const OutputBox = tw.div({
    base: "mt-3 p-3 rounded-lg bg-[var(--surface)] border border-[color-mix(in_srgb,var(--foreground)_10%,transparent)] font-mono text-xs",
    sub: { "span:key": "text-[var(--accent)]", "span:val": "text-[color-mix(in_srgb,var(--foreground)_70%,transparent)]" },
})

export const ObserverBox = tw.div({
    base: "rounded-xl border-2 p-4 transition-all duration-300",
    variants: {
        visible: {
            true: "border-emerald-400 bg-emerald-50 text-emerald-800",
