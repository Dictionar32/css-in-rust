...
// ─── Houdini-specific primitives ───────────────────────────────────────────────
export const ApiCard = tw.div({
    base: "rounded-xl border p-4 bg-[var(--surface)] flex flex-col gap-2",
    variants: {
        status: {
            stable: "border-emerald-200",
            experimental: "border-amber-200",
            limited: "border-red-200",
        },
    },
    defaultVariants: { status: "stable" },
    sub: {
        "span:badge": "text-[10px] font-semibold px-2 py-0.5 rounded-full self-start",
        "span:name": "text-sm font-bold",
...
export const SupportBadge = tw.span({
    base: "inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full",
    variants: {
        support: {
    ...
    defaultVariants: { support: "partial" },
})

export const SupportRow = tw.div({ base: "flex flex-wrap gap-2 my-3" })
